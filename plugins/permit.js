...
const ds = "Turns PM Permit permission."
const dds = "Turns off the PM Permit permission."
Asena.addCommand({pattern: 'approve$', desc: ds, onlyPm: true}, (async (message, match) => { 
    pmpermit.permitacton(message.jid.split("@")[0])
    await message.client.sendMessage(message.jid,'Approved for PM!', MessageType.text);
}));
Asena.addCommand({pattern: 'disapprove$', desc: dds, onlyPm: true}, (async (message, match) => { 
    pmpermit.nopermitacton(message.jid.split("@")[0])
    await message.client.sendMessage(message.jid,'Disapproved for pm!', MessageType.text);
}));
